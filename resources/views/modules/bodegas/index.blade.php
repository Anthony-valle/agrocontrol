    ...
    <section class="section">
      <div class="row">
        <div class="col-lg-12">
          ...
          <div class="card">
            <div class="card-body">
              <h5 class="card-title">Configuracion de Almacen</h5>
              ...
              <div class="d-flex flex-wrap justify-content-between align-items-center mb-3 p-2 bg-light rounded shadow-sm gap-3">
                  ...
                  <div class="d-flex align-items-center gap-3">
                      ...
                      <div class="input-group input-group-sm" style="max-width: 250px;">
                          ...
                          <input type="text" id="inputBusqueda" class="form-control border-start-0" placeholder="Buscar almacen...">
                      </div>
                  </div>
                  ...
                  <div>
                      <button type="button" class="btn btn-primary btn-sm" id="btnAbrirModal">
                          <i class="fa fa-plus me-1"></i> Nuevo Almacen
                      </button>
                  </div>
              </div>
              ...
              <div class="table-responsive border rounded" style="overflow-x:auto;">
                    <table class="table table-hover w-100 mb-0" id="tablaAlmacen">
                        ...
                        @foreach ($bodegas as $item)
                            <tr>
                                ...
                                <td><span class="badge {{ $item->estado ? 'bg-success' : 'bg-danger' }}">
                                        {{ $item->estado_texto }}
                                    </span>
                                </td>
                                <td>{{ $item->creador->usuario ?? 'Sistema' }}</td>           
                                <td class="text-nowrap">
                                    <a href="#" class="btn btn-warning btn-sm me-1 btnEditarAlmacen" data-id="{{ $item->id }}">
                                        <i class="fa-solid fa-pen-to-square"></i>
                                    </a>
                                    <button type="button" class="btn btn-danger btn-sm btnEliminarAlmacen" data-id="{{ $item->id }}">
...
<div class="modal fade" id="modalAlmacenEdit" tabindex="-1" aria-hidden="true">
    <div class="modal-dialog modal-lg modal-dialog-centered">
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    document.getElementById("btnAbrirModal").addEventListener("click", () => {
        fetch("{{ route('bodegas.create') }}")
            .then(res => res.text())
            .then(html => {
                document.getElementById("modalContent").innerHTML = html;
                new bootstrap.Modal(document.getElementById("modalAlmacen")).show();
                bindAjaxForm('modalAlmacen', 'modalContent', 'Bodega creada correctamente.');
            });
    });
    ...
    document.addEventListener('click', function(e){
        ...
        fetch(`/bodegas/${id}/edit`)
            .then(res => res.text())
            .then(html => {
                document.getElementById('modalContentEdit').innerHTML = html;
                new bootstrap.Modal(document.getElementById('modalAlmacenEdit')).show();
                bindAjaxForm('modalAlmacenEdit', 'modalContentEdit', 'Bodega actualizada correctamente.');
            });
    });

    // MODAL ELIMINAR
    document.addEventListener('click', function(e){
        const btn = e.target.closest('.btnEliminarAlmacen');
        if(!btn) return;
        ...
        Swal.fire({
            ...
            if(result.isConfirmed){
                ...
                .then(data => {
                    Swal.fire('Éxito', data.success || 'Bodega eliminada correctamente.', 'success').then(() => location.reload());
                })
                .catch(mostrarErrores);
            }
        });
    ...
    // Limpiar contenido de modales al cerrar y quitar backdrop
    const modalCrear = document.getElementById('modalAlmacen');
    modalCrear.addEventListener('hidden.bs.modal', () => {
        document.getElementById('modalContent').innerHTML = '';
        document.body.classList.remove('modal-open'); // Quitar clase modal-open
        const backdrops = document.querySelectorAll('.modal-backdrop');
        backdrops.forEach(b => b.remove()); // Eliminar cualquier backdrop residual
    });

    const modalEditar = document.getElementById('modalAlmacenEdit');
    modalEditar.addEventListener('hidden.bs.modal', () => {
        document.getElementById('modalContentEdit').innerHTML = '';
        document.body.classList.remove('modal-open'); // Quitar clase modal-open
        const backdrops = document.querySelectorAll('.modal-backdrop');
